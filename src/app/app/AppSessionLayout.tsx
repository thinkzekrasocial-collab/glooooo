...
export function AppSessionLayout({ children }: { children: ReactNode }) {
  ...
  const loadSession = useCallback(async () => {
    ...
    try { setMe(await apiFetch<RemoteMe>("/api/users/me")); }
    catch (error) {
      if (error instanceof ApiClientError && (error.status === 401 || error.status === 403)) { router.replace("/login"); return; }
  ...
  return <AppShell me={{ id: current.id, name: current.preferredName || current.name || current.firstName || current.email || "Messenger", email: current.email ?? "", userType: current.userType ?? "member", roles: me.roles ?? [], permissions, canAdmin, mfaEnabled: current.mfaEnabled ?? false, mfaRequired: current.mfaRequired ?? false }} unreadNotifications={0}>{children}</AppShell>;
}
