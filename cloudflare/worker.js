const cors = (request, env) => {
  // Keep the allow-list explicit. Set FRONTEND_ORIGIN to the deployed web
  // app's origin; the Worker origin remains the safe default for its bundled
...
let me=null;const $=id=>document.getElementById(id);const call=async(path,opts={})=>{const r=await fetch('/api'+path,{credentials:'same-origin',...opts,headers:{'content-type':'application/json',...(opts.headers||{})}});const j=await r.json();if(!r.ok)throw Error(j.error||'Request failed');return j};
async function load(){try{me=await call('/me');$('auth').hidden=true;$('chat').hidden=false;await refresh()}catch{me=null;$('auth').hidden=false;$('chat').hidden=true}}
...
$('login').onclick=async()=>{try{await call('/login',{method:'POST',body:JSON.stringify({email:$('email').value,password:$('password').value})});$('authStatus').textContent='Signed in';await load()}catch(e){$('authStatus').textContent=e.message}};
$('signup').onclick=()=>{$('authStatus').textContent='Public registration is disabled.'};
$('send').onclick=async()=>{const v=$('message').value.trim();if(!v)return;try{await call('/messages',{method:'POST',body:JSON.stringify({ciphertext:v})});$('message').value='';await refresh()}catch(e){$('status').textContent=e.message}};$('message').onkeydown=e=>{if(e.key==='Enter')$('send').click()};$('refresh').onclick=refresh;$('logout').onclick=async()=>{try{await call('/auth/logout',{method:'POST'})}finally{location.reload()}};load();
</script></body></html>`;

const tokenFor = () => crypto.randomUUID() + crypto.randomUUID();
async function userFrom(request, env) {
  const cookieToken = request.headers.get("cookie")?.match(/(?:^|;\s*)gb_session=([^;]+)/)?.[1];
  const token = cookieToken || request.headers.get("authorization")?.replace(/^Bearer\s+/i, "");
...
async function seedAccounts(env) {
  ...
  seedPromise = (async () => {
  const accounts = [
    ["admin@globebridge.edu", env.DEMO_ADMIN_PASSWORD, "admin"],
    ["marcus.lee@globebridge.edu", env.DEMO_MEMBER_PASSWORD, "member"],
    ["alina.rahman@globebridge.edu", env.DEMO_MEMBER_PASSWORD, "member"],
...
const worker = { async fetch(request, env) {
  ...
  const json = (body, status = 200, extra = {}) =>
    jsonResponse(body, status, { ...corsHeaders, ...extra });
  ...
  if (path === "/auth/logout" && request.method === "POST") {
    const token = request.headers.get("cookie")?.match(/(?:^|;\s*)gb_session=([^;]+)/)?.[1] || request.headers.get("authorization")?.replace(/^Bearer\s+/i, "");
    if (token) await env.DB.prepare("delete from sessions where token=?").bind(await tokenHash(token)).run();
    return json({ ok: true }, 200, { "set-cookie": "gb_session=; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=0" });
  }
  ...
  if (path === "/messages" && request.method === "GET") { const r=await env.DB.prepare("select m.*,u.email from messages m join users u on u.id=m.user_id where m.user_id=? order by m.created_at asc limit 500").bind(user.id).all(); return json({messages:r.results}); }
...
async function verifyPassword(password,stored){try{return await bcrypt.compare(password,stored);}catch{return false;}}
